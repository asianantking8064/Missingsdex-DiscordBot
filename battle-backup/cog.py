...
class GuildBattle:
    ...
    abilities: Dict[str, str] = field(default_factory=dict)  # Ability customization
...
def update_embed(
    ...
    embed = discord.Embed(
        title=f"{settings.collectible_name.title()} Battle Plan",
        description=(
            f"Add or remove {settings.collectible_name} you want to propose to the other player using the "
...
class Battle(commands.GroupCog):
    ...
    async def start(self, interaction: discord.Interaction, opponent: discord.Member):
        ...
        await interaction.response.send_message(
            f"Hey, {opponent.mention}, {interaction.user.name} threatening to make your balls go missing!",
            embed=embed,
            view=view,
        )
