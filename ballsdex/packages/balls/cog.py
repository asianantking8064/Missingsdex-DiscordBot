...
class DuplicateType(enum.StrEnum):
    countryballs = settings.collectible_name
    specials = "specials"
...
class Balls(commands.GroupCog, group_name=settings.players_group_cog_name):
    ...
    async def compare(
        ...
        try:
            ...
            await interaction.followup.send(
                f"{user.display_name} doesn't have any {settings.collectible_name} yet."
            )
            return
        ...
        source.embed.title = (
            ...
            f"{settings.collectible_name}{special_str}"
        )
        source.embed.colour = discord.Colour.blurple()
        ...
        if not balls:
            if countryball:
                await interaction.followup.send(
                    ...
                    f"{settings.collectible_name} yet."
                )
            else:
